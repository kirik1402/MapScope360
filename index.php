...
$cacheFile = 'cache/frames_list.json';

// Загружаем данные из кэшированного файла
$frameData = json_decode(file_get_contents($cacheFile), true);

if (empty($frameData)) {
    die("No frames found in the cache.");
}

// Используем первый файл как начальный
$firstFrame = 'uploads/frames/' . $frameData[0]['file'];
?>
...
<head>
    ...
    <style>
        ...
        /* Новый стиль для метки даты и времени */
        #timestamp {
            position: absolute;
            top: 10px;
            left: 10px;
...
<body>
    <div id="panorama-container">
        <div id="panorama"></div>
        <div id="timestamp">Дата и время съёмки</div> <!-- Новый элемент для отображения времени съёмки -->
    </div>
    ...
    <script>
        ...
        // Массив с информацией обо всех фреймах
        var frames = <?php echo json_encode($frameData); ?>;
        var map;
        ...
        function loadPanorama(imageUrl, updateUrl = true) {
            ...
            if (frame && frame.timestamp) {
                document.getElementById('timestamp').textContent = `Дата и время съёмки: ${frame.timestamp}`;
            }
        ...
        function normalizeYaw(yaw) {
            // Приводим yaw к положительному значению в диапазоне 0-360 градусов
        ...
        function loadPanoramaFromURL() {
            ...
            if (frame) {
                ...
                loadPanorama("<?php echo $firstFrame; ?>");
            }
        }

        window.onpopstate = function(event) {
            if (event.state && event.state.file) {
                const frame = frames.find(f => f.file === event.state.file);
                currentYaw = event.state.yaw || 30;
                currentPitch = event.state.pitch || 0;
                currentHfov = event.state.hfov || currentHfov;
                if (frame) {
                    loadPanorama('uploads/frames/' + frame.file, false);
                }
            }
